...
class MultiWozReader(_ReaderBase):
    ...
    def get_exp_domains(self, exp_domains, all_domains_list):
        if 'Camera' in exp_domains:
            if 'except' in exp_domains:
                # ['except', 'hotel']
                domains = [d for d in all_domains_list if 'Camera' not in d and 'multi' not in d]
            else:
                # ['hotel']
                domains = ['Camera_single', 'Camera_multi']
        if 'Phone' in exp_domains:
            if 'except' in exp_domains:
                # ['except', 'train']
                domains = [d for d in all_domains_list if 'Phone' not in d and 'multi' not in d]
            else:
                # ['train']
                domains = ['Phone_single', 'Phone_multi']
        if 'Tablet' in exp_domains:
            if 'except' in exp_domains:
                # ['except', 'attraction']
                domains = [d for d in all_domains_list if 'Tablet' not in d and 'multi' not in d]
            else:
                # ['attraction']
                domains = ['Tablet_single', 'Tablet_multi']
        if 'Computer' in exp_domains:
            if 'except' in exp_domains:
                # ['except', 'restaurant']
                domains = [d for d in all_domains_list if 'Computer' not in d and 'multi' not in d]
            else:
                # ['restaurant']
                domains = ['Computer_single', 'Computer_multi']
        if 'Laptop' in exp_domains:
            if 'except' in exp_domains:
                # ['except', 'taxi']
                domains = [d for d in all_domains_list if 'Laptop' not in d and 'multi' not in d]
            else:
                # ['taxi']
                domains = ['Laptop_single', 'Laptop_multi']
        return domains
    ...
    def _construct_bspn_constraint(self):
        bspn_masks = {}
        valid_domains = ['Phone','Camera','Laptop','Computer','Tablet']
        ...
        with open('PPMD/ppmd-processed/bspn_masks.txt', 'w') as f:
    ...
    def _construct_aspn_constraint(self):
        ...
        with open('PPMD/ppmd-processed/aspn_masks.txt', 'w') as f:
