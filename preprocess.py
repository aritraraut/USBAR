...
def get_db_values(value_set_path): # value_set.json, all the domain[slot] values in datasets
    ...
    with open('PPD/ppd-processed/bspn_word_collection.json', 'w') as f:
...
class DataPreprocessor(object):
    def __init__(self):
        ...
        data_path = 'PPD/data.json'
        archive = zipfile.ZipFile(data_path + '.zip', 'r')
        self.convlab_data = json.loads(archive.open(data_path.split('/')[-1], 'r').read())#.lower())
        self.delex_sg_valdict_path = 'PPD/ppd-processed/delex_single_valdict.json'
        self.delex_mt_valdict_path = 'PPD/ppd-processed/delex_multi_valdict.json'
        self.ambiguous_val_path = 'PPD/ppd-processed/ambiguous_values.json'
        self.delex_refs_path = 'PPD/ppd-processed/reference_no.json'
    ...
    def preprocess_main(self, save_path=None, is_test=False):
        ...
        self.vocab.save_vocab('PPD/ppd-processed/vocab')
        with open('PPD/ppd-analysis/dialog_acts.json', 'w') as f:
            json.dump(ordered_sysact_dict, f, indent=2)
        with open('PPD/ppd-analysis/dialog_act_type.json', 'w') as f:
...
if __name__=='__main__':
    ...
    if not os.path.exists('PPD/ppd-processed'):
        os.mkdir('PPD/ppd-processed')

    with open('PPD/ppd-processed/data_for_damd.json', 'w') as f:
